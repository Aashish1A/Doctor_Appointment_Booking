  ...
  return (
    <form onSubmit={onSubmitHandler} className='min-h-screen flex items-center'>
        <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-[#5e5e5e] text-sm shadow-lg'>
            ...
            <button className='bg-primary text-white rounded w-full p-2 mt-3 cursor-pointer'>Login</button>
            {
                state === "Admin" ?
                <p className='text-sm m-auto'>Doctor Login? <span onClick={() => setState("Doctor")} className='text-primary underline cursor-pointer'>Click here</span></p>
                :
                <p className='text-sm m-auto'>Admin Login? <span onClick={() => setState("Admin")} className='text-primary underline cursor-pointer'>Click here</span></p>
            }
        </div>
    </form>
  )
